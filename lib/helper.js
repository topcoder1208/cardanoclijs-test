...
exports.txInToString = (dir, txInList, isCollateral) => {
  ...
  txInList.forEach(
    (txIn) =>
    (result += `${txIn.txHash !== undefined && txIn.txId !== undefined ? '--tx-in' : ''}${isCollateral ? "-collateral" : ""} ${txIn.txHash || ''}${txIn.txHash ? '#' : ''}${txIn.txId === undefined ? '' : txIn.txId
      } ${txIn.script
        ? `--tx-in-script-file ${this.jsonToPath(dir, txIn.script)} `
        : ""
      } ${txIn.datum
        ? `--tx-in-datum-file ${this.jsonToPath(dir, txIn.datum)} `
        : ""
      } ${txIn.redeemer
        ? `--tx-in-redeemer-file ${this.jsonToPath(dir, txIn.redeemer)} `
        : ""
      } ${txIn.executionUnits
...
exports.txOutToString = (dir, txOutList) => {
  ...
  txOutList.forEach((txOut) => {
    assetOutStr = "";
    if (txOut.datumEmbed) {
      result += `--tx-out-datum-embed-file ${this.jsonToPath(dir, txOut.datumEmbed)}`
    } else {
      ...
      txOut.datumEmbed && (result += `--tx-out-datum-embed-file ${this.jsonToPath(dir, txOut.datumEmbed)}`)
    }
    result += " ";
  });
  return result;
  ...
  result += minting
    .map((mint) => {
      if (mint.script) {
        ...
        return `--mint-redeemer-file ${this.jsonToPath(dir, mint.redeemer)}`
