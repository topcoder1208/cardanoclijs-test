...
exports.certToString = (dir, certList) => {
  ...
  certList.forEach(
    (cert) =>
    (result += `--certificate ${cert.cert} ${cert.script
      ...
      : ""
        ...
        ? `--certificate-execution-units "(${cert.executionUnits[0] + "," + cert.executionUnits[1]
        })" `
        : ""
      }`)
  );
...
exports.withdrawalToString = (dir, withdrawalList) => {
  ...
  withdrawalList.forEach(
    ...
    (result += `--withdrawal ${withdrawal.stakingAddress}+${withdrawal.reward
        ...
        ? `--withdrawal-execution-units "(${withdrawal.executionUnits[0] + "," + withdrawal.executionUnits[1]
        })" `
        : ""
      }`)
  );
...
exports.txInToString = (dir, txInList, isCollateral) => {
  ...
  txInList.forEach(
    ...
    (result += `--tx-in${isCollateral ? "-collateral" : ""} ${txIn.txHash}#${txIn.txId
        ...
        ? `--tx-in-script-file ${txIn.isPath ? txIn.script : this.jsonToPath(dir, txIn.script)} `
        : ""
      } ${txIn.datum
        ? `--tx-in-datum-file ${txIn.isPath ? txIn.datum : this.jsonToPath(dir, txIn.datum)} `
        : ""
      } ${txIn.redeemer
        ? `--tx-in-redeemer-file ${txIn.isPath ? txIn.redeemer : this.jsonToPath(dir, txIn.redeemer)} `
        : ""
      } ${txIn.executionUnits
        ? `--tx-in-execution-units "(${txIn.executionUnits[0] + "," + txIn.executionUnits[1]
        })" `
        : ""
      }`)
  );
...
exports.txOutToString = (dir, txOutList) => {
  let result = "";
  let assetOutStr;
  txOutList.forEach((txOut) => {
    ...
    txOut.datumEmbed && (result += ` --tx-out-datum-embed-file ${txOut.isPath ? txOut.datumEmbed : this.jsonToPath(dir, txOut.datumEmbed)}`)
    result += " ";
  });
  return result;
};
...
exports.mintToString = (dir, minting) => {
  ...
  minting.forEach((mint, index, arr) => {
    ...
    result += (flag ? '+' : '') + `${mint.action == "mint" ? "" : "-"}${mint.quantity} ${mint.asset}`;
    flag = true;
  });
  result = result.trim();
  result += `" `;
    ...
    .map((mint) => {
      if (mint.script) {
        if (mint.isPath) {
          ...
          return `--mint-script-file ${mint.script}`;
        }
      ...
      } else if (mint.redeemer) {
        return `--mint-redeemer-file ${mint.isPath ? mint.redeemer : this.jsonToPath(dir, mint.redeemer)}`
      } else if (mint.executionUnits) {
        return `--mint-execution-units "(${mint.executionUnits[0] + "," + mint.executionUnits[1]})" `
      }
    })
    .join(" ");
  return result;
