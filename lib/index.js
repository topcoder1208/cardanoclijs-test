...
class CardanocliJs {
  ...
  nodeIssueOpCert(poolName, kesPeriod) {
    ...
    execSync(`${this.cliPath} node issue-op-cert \
                        --kes-verification-key-file ${this.dir
      }/priv/pool/${poolName}/${poolName}.kes.vkey \
                        --cold-signing-key-file ${this.dir
      }/priv/pool/${poolName}/${poolName}.node.skey \
                        --operational-certificate-issue-counter ${this.dir
  ...
  transactionBuildRaw(options) {
    ...
    const txOutString = txOutToString(this.dir, options.txOut);
    ...
    execSync(`${this.cliPath} transaction build-raw \
                ...
                ${metadata} \
                ${scriptInvalid} \
                --invalid-hereafter ${options.invalidAfter
  ...
  transactionBuild(options) {
    ...
    const witnessOverride = options.witnessOverride
      ...
      : "";
    const alonzoEra = options.alonzoEra
      ? `--alonzo-era`
      : "";
    if (!this.protocolParametersPath) this.queryProtocolParameters();
   ...
   * @returns {string} Datum hash
   */
  transactionHashStringScriptData(script) {
    if (this.httpProvider && typeof window !== "undefined") {
      let response = fetch(`${this.httpProvider}/transactionHashScriptData`, {
        headers: {
          "Content-Type": "application/json",
    ...
    return execSync(
      `${this.cliPath
      } transaction hash-script-data --script-data-value '${script}'`
    )
      .toString()
      .trim();
  }
